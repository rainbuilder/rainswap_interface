...
import { getCurrentRewardPerBlock } from '../../../sushi/utils'
...
const FarmCards: React.FC = () => {
  ...
  const rows = farms.reduce<FarmWithStakedValue[][]>(
    (farmRows, farm, i) => {
      ...
      // console.log('farm with stake', farmWithStakedValue.usdValue.toString())
      if (newFarmRows[newFarmRows.length - 1].length === 3) {
        newFarmRows.push([farmWithStakedValue])
      } else {
        newFarmRows[newFarmRows.length - 1].push(farmWithStakedValue)
      }
      // console.log('new farmer rows', farmWithStakedValue.usdValue.toString())
      return newFarmRows
    },
    [[]]
  )
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const supply = await getCurrentRewardPerBlock(sushi, farm.pid)
  ...
  function formatApyValue(apyValue: any) {
    if (isNaN(apyValue)) return 0
    return apyValue
  }

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'RAIN' && <StyledCardAccent />}
      <CardWrap>
        <CardContent>
          <StyledContent>
            <StyledTopIcon>
              ...
              <>
                <StyledInsight>
                  ...
                  <span>
                    {newReward && (
                      <>
                        <b>{getBalanceNumber(newReward).toFixed(2)} RAIN</b> / block
                      </>
                    )}
                    {!newReward && '~'}
                  </span>
