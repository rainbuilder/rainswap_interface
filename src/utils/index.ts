...
import { ChainId as UChainId, JSBI, Percent, Token, CurrencyAmount, Currency, ETHER, TOMO } from '@luaswap/sdk'
import { ETHER as BNB, ChainId as PChainId } from '@pancakeswap-libs/sdk'
import { TokenAddressMap } from '../state/lists/hooks'
import EthereumLogo from '../assets/images/ethereum-logo.png'
import TomoLogo from '../assets/images/tomo-logo.png'
import BnbLogo from '../assets/images/bnb-logo.png'

type ChainId = UChainId | PChainId
...
export function isBinanceChain(chainId: ChainId | undefined) {
  return chainId === 56 || chainId === 97
}

export function getNativeToken(chainId: ChainId | undefined) {
  const IsTomo = IsTomoChain(chainId)
  const isBinance = isBinanceChain(chainId)
  if (IsTomo) {
    return TOMO
  } else if (isBinance) {
    return BNB
