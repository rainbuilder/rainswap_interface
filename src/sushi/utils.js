...
export const getRainMasterChefAddress = rain => {
  return rain && rain.rainMasterChefAddress
}
export const getRainAddress = rain => {
  return rain && rain.rainAddress
}
...
export const getRainMasterChefContract = rain => {
  return rain && rain.contracts && rain.contracts.rainMasterChef
}

export const getRainContract = rain => {
  return rain && rain.contracts && rain.contracts.rain
}
...
// TODO Add call api for getting asset price
export const getLPValue = async (masterChefContract, lpContract, tokenContract, token2Contract, pid) => {
  ...
  const tokenAmountWholeLP = await tokenContract.methods.balanceOf(lpContract.options.address).call() // RAIN TOken Amount in CAKE-LP POOL
  const tokenDecimals = await tokenContract.methods.decimals().call()
  // Get the share of lpContract that masterChefContract owns
  const balance = await lpContract.methods.balanceOf(masterChefContract.options.address).call() // RAIN TOken Amount in CAKE-LP POOL

  // Convert that into the portion of total lpContract = p1
  const totalSupply = await lpContract.methods.totalSupply().call()

  // Get total token2 value for the lpContract = w1
  const lpContractToken2 = await token2Contract.methods.balanceOf(lpContract.options.address).call() // BUSD TOken Amount in CAKE-LP POOL
  ...
  if (
    ...
    token2Contract._address.toLowerCase() === busdAddress
  ) {
    usdValue = totalToken2Value
  } else if (
    token2Contract._address.toLowerCase() === wethAddress ||
    token2Contract._address.toLowerCase() === wbnbAddress
  ) {
...
export const getNewRewardPerBlock = async (sushi, pid1 = 0, chainId) => {
  if (pid1 === 0) {
    ...
    if (await checkPoolActive(pid1 - 1, chainId)) {
      ...
      console.log(
        'rewared block',
        new BigNumber(await UnknownBlock(chef._address, 'getNewRewardPerBlock(uint256):(uint256)', [1], true, chainId))
      )
      return new BigNumber(
        await UnknownBlock(chef._address, 'getNewRewardPerBlock(uint256):(uint256)', [pid1], true, chainId)
      )
...
export async function getCurrentRewardPerBlock(sushi, pid1 = 0) {
  const chef = getMasterChefContract(sushi)
  const rewards2 = await chef.methods.getNewRewardPerBlock(pid1).call()

  // console.log('reward called', rewards2)
  return new BigNumber(rewards2)
}
